...
- **Writing the exhaustive project documentation honestly.** A stray all_users.txt file at the project root, holding Python-syntax data but never imported by the actual script, took some digging to explain honestly: the evidence points to it being a leftover from before the tool's hardcoded-list refactor, not a currently-used input, and it's documented as inferred rather than confirmed.
